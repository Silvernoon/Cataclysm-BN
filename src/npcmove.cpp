                      ...
                      int range, const tripoint_bub_ms &danger_pos )
{
    ...
    const auto warning_name = "warning_" + type + name;
    auto speech = snip;
    if( !name.empty() ) {
        const auto range_str = range < 1 ? "<punc>" :
                               string_format( _( " %s, %s" ),
                                              direction_name( direction_from( bub_pos(), danger_pos ) ),
                                              distance_string( range ) );
        speech = string_format( _( "%s %s%s" ), snip, name, range_str );
    }
    if( cata::has_hooks( "on_npc_warning" ) ) {
        const auto hook_results = cata::run_hooks( "on_npc_warning",
        [ &, this]( sol::table & params ) {
            params["npc"] = this;
            params["warning_type"] = type;
            params["speech"] = speech;
        } );
        speech = hook_results.get_or( "speech", speech );
    }
